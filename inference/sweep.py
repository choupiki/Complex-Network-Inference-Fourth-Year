...
# let's give this the model?
class MonteCarlo2:
    def __init__(self, model, modparams, fout):
        self.fout = fout
        self.modparams = modparams
        self.model = model
        self.N, _ = model.shape
    ...
    def run(self):
        # print(self.fout)
        # print("Sampling every {} cycles".format(self.cycle_dumpfreq))
        # print("EQ: {} cycles".format(self.eq_cycles))
        # print("PROD: {} cycles".format(self.prod_cycles))
        ...
        # print('----')
        # print(samples_tot)
        # theres potentail for this to go wrong somehwere around here!
        # i.e. that the shapes don't correspond and stuff like that!
        with h5py.File(self.fout, "a") as f:

            initial_config = utils.initialise_ising_config(self.N, 0)

            eq_traj, eq_energy = mc.simulate(
                self.model, initial_config,
                self.eq_cycles, self.cycle_dumpfreq)
            eq_config = eq_traj[-1]

            prod_traj, prod_energy = mc.simulate(
                self.model, eq_config,
                self.prod_cycles, self.cycle_dumpfreq)

            # so here I've got smallish stuff.
            if "/energy" in f:
                del f["energy"]
            if "/configurations" in f:
                del f["configurations"]

            energy_ds = f.require_dataset(
                "energy",
                shape=(samples_tot),
                dtype=prod_energy.dtype,
                compression="gzip")

            config_ds = f.require_dataset(
                "configurations",
                shape=(samples_tot, self.N),
                dtype=prod_traj.dtype,
                compression="gzip")
            ...
            config_ds.attrs['file'] = self.fout
            config_ds.attrs['N'] = self.N
            config_ds.attrs['eq_cycles'] = self.eq_cycles
            config_ds.attrs['prod_cycles'] = self.prod_cycles
            config_ds.attrs['cycle_dumpfreq'] = self.cycle_dumpfreq

            for key, val in self.modparams.items():
                config_ds.attrs[key] = val
