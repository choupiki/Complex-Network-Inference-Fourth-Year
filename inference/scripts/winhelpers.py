...
def genSK(parallel_arguments):
    # print(parallel_arguments)
    T = parallel_arguments[0]
    h = parallel_arguments[1]
    J0 = parallel_arguments[2]
    Jstd = parallel_arguments[3]
    ...
    model = utils.SK_interaction_matrix(N, T, h, J0, Jstd)              # make array for specific model u want to use - should be a straight swap e.g o_utils.SM_matrix(40, 2, 1, 0, 1) 
...
def mccSK(h5py_fname, mcc_args):
    with h5py.File(h5py_fname, 'r') as fin:     #set metadata
        ...
        modparams['J'] = model_dataset.attrs['J0']
        modparams['Jstd'] = model_dataset.attrs['Jstd']
