...
def genCM(parallel_arguments):
    # print(parallel_arguments)
    T = parallel_arguments[0]
    h = parallel_arguments[1]
    _type = parallel_arguments[2]
    ...
    model, graph = utils.complete_matrix(N, h, T)             # make array for specific model u want to use - should be a straight swap e.g o_utils.SM_matrix(40, 2, 1, 0, 1) 
    plt.imshow(model)
    plt.show()
    """
    nx.draw_kamada_kawai(graph, **{'node_size': 1, 'width': 0.15})
    os.makedirs("C:/Users/oscar/Documents/School Stuff/Physics/4th Year/Research Project/Images/CM_Nets/" + str(outdir) + "/")
    plt.savefig("C:/Users/oscar/Documents/School Stuff/Physics/4th Year/Research Project/Images/CM_Nets/" + str(outdir) + "/" + str(N) + ".tiff", dpi=1000, format='tiff')
    plt.show()
    """
    h5py_fname = os.path.join(outdir, modlabel)
    h5py_fname = h5py_fname + '.hdf5'
...
def gen2D(parallel_arguments):
    ...
    model, graph = utils.twoD_ising_matrix(N, h, T)             # make array for specific model u want to use - should be a straight swap e.g o_utils.SM_matrix(40, 2, 1, 0, 1) 
    
    plt.imshow(model)
    plt.show()
    nx.draw(graph, **{'node_size': 1, 'width': 0.15})
    plt.savefig("C:/Users/oscar/Documents/School Stuff/Physics/4th Year/Research Project/Images/2D_Nets/" + str(outdir) + ".tiff", dpi=600, format='tiff')
    plt.show()
...
def genSF(parallel_arguments):
    ...
    plt.title(gamma)
    nx.draw_kamada_kawai(graph, **{'node_size': 1, 'width': 0.15})
    plt.savefig("C:/Users/oscar/Documents/School Stuff/Physics/4th Year/Research Project/Images/SF_Nets/" + str(outdir) + "/" + str(gamma) + ".png", dpi=500, format='png')
    plt.show()
    h5py_fname = os.path.join(outdir, modlabel)
    h5py_fname = h5py_fname + '.hdf5'
    with h5py.File(h5py_fname, "a") as f:
...
def mccCM_2D(h5py_fname, mcc_args):
    with h5py.File(h5py_fname, 'r') as fin:                      # set metadata
        model_dataset = fin['InputModel']
        model = model_dataset[:]
        modparams = {}
